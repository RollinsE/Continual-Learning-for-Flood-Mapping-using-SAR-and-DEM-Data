...
def _spline_2d(window_size: int, power: int = 2) -> torch.Tensor:
    ...
    """
    # Cache spline windows because the same shape is reused across tiles.
    global WINDOW_CACHE
...
def undo_rotate_and_mirror(variants: List[torch.Tensor]) -> torch.Tensor:
    ...
    This restores transformed inputs to the base orientation, then averages them.
...
def predict_smooth_windowing(image: torch.Tensor,
    ...
    Args:
        ...
        mirrored (bool, optional): whether to use all dihedral rotations/reflections. Defaults to False.
