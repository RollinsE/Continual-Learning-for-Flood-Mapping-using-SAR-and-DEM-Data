...
def mask_raster(path: Path, mask: np.ndarray, mask_value: int = 0) -> None:
    ...
    with rasterio.open(str(path), mode="w", **profile) as dst:
        if current.shape[1:] != mask.shape:
            raise ValueError(f"Mask shape {mask.shape} does not match raster shape {current.shape[1:]}")
...
def rgb_ratio(sar_image: np.ndarray,
    ...
    if sar_image.ndim != 3:
        raise ValueError(f"Expected one 3D SAR image, got shape {sar_image.shape}")
    if channels_first:
