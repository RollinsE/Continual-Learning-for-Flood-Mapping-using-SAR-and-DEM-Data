...
class ASPPModule(nn.Module):
    ...
    def __init__(self,
        ...
        # The pooled branch is resized dynamically in forward() so full-scene
        # and variable-sized rasters are supported during evaluation.
...
class DecoderV3Plus(nn.Module):
    ...
    features (higher resolution) into the highly dimensional output.
    Reference paper: https://arxiv.org/abs/1802.02611
    """
    def __init__(self,
                 skip_channels: int,
                 lower_upscale: int,
        ...
        # Keep the nominal upscale value for configuration introspection,
        # but use dynamic interpolation in forward() so odd or full-scene raster
        # sizes align exactly with the skip branch.
        self.lower_upscale = int(lower_upscale)

        # Decoder refinement block with two 3x3 convolutions, yapf: disable
    ...
    def forward(self, x: torch.Tensor, skip: torch.Tensor) -> torch.Tensor:
        ...
        with the upsampled high-level features 'x'. The output is resized to the
...
class DeepLabV3(Decoder):
    ...
    def __init__(self,
        ...
        if not (len(feature_channels) == len(feature_reductions) == 1):
            raise ValueError("DeepLabV3 requires exactly the final encoder output")
...
class DeepLabV3Plus(Decoder):
    ...
    def __init__(self,
        ...
        super().__init__(input_size, feature_channels, feature_reductions, act_layer, norm_layer)
        if not (len(feature_channels) == len(feature_reductions) == 2):
            raise ValueError("DeepLabV3+ requires the final encoder output and one skip connection")
