...
class ResNet(nn.Module):
    """Torchvision ResNet backbone adapted for segmentation."""
    def __init__(self, in_channels=3, output_stride=16, backbone='resnet101', pretrained=True):
        super().__init__()
        if output_stride not in {8, 16}:
            raise ValueError(f"output_stride must be 8 or 16, got {output_stride}")
        if not hasattr(models, backbone):
            raise ValueError(f"Unsupported torchvision ResNet backbone: {backbone}")
        constructor = getattr(models, backbone)
        weights = models.get_model_weights(backbone).DEFAULT if pretrained else None
...
# Aligned Xception backbone adapted from Cadene/pretrained-models.pytorch.
...
class Xception(nn.Module):
    def __init__(self, output_stride=16, in_channels=3, pretrained=True):
        ...
        # Stride for block 3 (entry flow), and dilation rates for middle and exit flows.
        if output_stride == 16:
        ...
        # Middle flow.
        for i in range(16):
            setattr(self, f"block{i + 4}", Block(728, 728, stride=1, dilation=mf_d))
        ...
        for k, v in pretrained_weights.items():
            if k in state_dict:
                ...
                if k.startswith('block11'):
                    # Replicate pretrained middle-flow weights across the expanded block set.
                    model_dict[k] = v
                    for i in range(8):
...
def aspp_branch(in_channels, out_channels, kernel_size, dilation):
    """Build one atrous spatial pyramid pooling branch."""
...
class Decoder(nn.Module):
    ...
    def forward(self, x, low_level_features):
        ...
        height, width = low_level_features.size(2), low_level_features.size(3)
...
class DeepLab(BaseModel):
    def __init__(self,
        ...
        super().__init__()
        if backbone != "xception" and not backbone.startswith("resnet"):
        ...
        # Keep the historical attribute name ``ASSP`` so existing checkpoints retain
        # the same state-dict key prefix; the implementation itself uses the correct ASPP name.
        self.ASSP = ASPP(in_channels=backbone_out_channels, output_stride=output_stride)
    ...
    # Parameter groups allow callers to assign separate learning rates to the
    # feature extractor and decoder components.
