...
def valid_samples(ignore_index: int,
    ...
    if y_pred is not None:
        if y_pred.shape != y_true.shape:
            raise ValueError(f"Prediction/target shape mismatch: {y_pred.shape} != {y_true.shape}")
...
def confusion_matrix(y_true: torch.Tensor, y_pred: torch.Tensor, ignore_index: Optional[int] = None) -> torch.Tensor:
    ...
    Returns:
        torch.Tensor: Confusion matrix ``C`` with shape ``[num_classes, num_classes]``.
    """
    flat_target = y_true.view(-1)
    flat_pred = y_pred.view(-1)
    # Exclude pixels marked with the ignore index.
    if ignore_index is not None:
        flat_target, flat_pred = valid_samples(ignore_index, y_true=y_true, y_pred=y_pred)
    # Use bins to compute the confusion matrix efficiently.
...
def binary_confusion_matrix(y_true: torch.Tensor,
    ...
    if ignore_index is not None:
        flat_target, flat_pred = valid_samples(ignore_index, y_true=y_true, y_pred=y_pred)
    # Compute each confusion-matrix cell directly for binary masks.
...
def iou_score(tp: torch.Tensor, fp: torch.Tensor, fn: torch.Tensor, reduce: bool = True) -> torch.Tensor:
    ...
    :param fn: false negatives, with the same shape convention as TP and FP
...
def f1_score(tp: torch.Tensor, fp: torch.Tensor, fn: torch.Tensor, reduce: bool = True) -> torch.Tensor:
    ...
    :rtype: torch.Tensor
    """
    # Keep sub-metrics unreduced so macro F1 is computed from class-wise precision and recall.
...
def matthews_corrcoef(tp: torch.Tensor, fp: torch.Tensor, tn: torch.Tensor, fn: torch.Tensor, reduce: bool = True) -> torch.Tensor:
    """Compute Matthews correlation coefficient from binary confusion statistics.

    Confusion counts can be very large for segmentation rasters, so the arithmetic
    is performed in float64 to avoid integer overflow in the denominator product.
    """
    ...
    denominator = torch.sqrt(torch.clamp((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn), min=0.0))
    score = numerator / torch.clamp(denominator, min=float(F32_EPS))
    score = torch.nan_to_num(score, nan=0.0, posinf=0.0, neginf=0.0).to(torch.float32)
    return score.mean() if reduce else score
