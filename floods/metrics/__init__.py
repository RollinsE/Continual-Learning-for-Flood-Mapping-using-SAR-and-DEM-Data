...
def lenient_sigmoid(*args: Iterable[torch.Tensor]) -> Iterable[torch.Tensor]:
    result = list()
    for tensor in args:
        # Binary segmentation models often return [B, 1, H, W] logits while
        # masks are [B, H, W]. Squeeze the singleton channel so metrics use the
        # same shape as the ground truth.
        if tensor.ndim == 4 and tensor.shape[1] == 1:
            tensor = tensor[:, 0]
        tensor = (torch.sigmoid(tensor.float()) > 0.5).int()
        result.append(tensor)
    return result
...
class GeneralStatistics(Metric):
    ...
    def update(self, y_true: torch.Tensor, y_pred: torch.Tensor) -> None:
        ...
        Args:
            ...
            y_pred (torch.Tensor): prediction batch, before transformation into indices, with size [B, C, H, W]
        """
        ...
        onehot_true = (flat_true > 0).float()
        onehot_pred = flat_pred
    ...
    def compute(self) -> torch.Tensor:
        ...
        # Support is the number of true positive-class pixels.
...
class Precision(GeneralStatistics):
    ...
    Precision measures the proportion of predicted positives that are correct. A value of 1.0 means every positive prediction is correct, although true positives may still be missed; recall captures that complementary behaviour.
    """
    def compute(self) -> torch.Tensor:
        """Compute the final precision score from accumulated statistics.
...
class MCC(GeneralStatistics):
    """Matthews correlation coefficient for binary flood segmentation."""

    def compute(self) -> torch.Tensor:
        if self.background:
            score = func.matthews_corrcoef(
                tp=self.tn,
                fp=self.fn,
                tn=self.tp,
                fn=self.fp,
                reduce=self.reduce_first,
            )
        else:
            score = func.matthews_corrcoef(
                tp=self.tp,
